...
function getUserGames(steamId) {
    ...
    .then((data) => ({
        steamId,
        gameIds: data.response.games && data.response.games.map((game) => game.appid),
    }));
}
...
function getAllUsersGames(steamIds) {
    // for each steamId
    // get user games
    const gameArray = steamIds.split(',').map((steamId) => getUserGames(steamId));
...
function getGameInfo(gameId) {
    ...
    .then((data) => {
        return ({
            gameName: data[gameId] && data[gameId].data && data[gameId].data.name,
            gameImage: data[gameId] && data[gameId].data && data[gameId].data.header_image
        })
    });
}
